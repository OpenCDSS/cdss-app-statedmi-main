// TranslatePenmanMonteith_Command - This class initializes, checks, and runs the TranslatePenmanMonteith() command.
...
/**
<p>
This class initializes, checks, and runs the TranslatePenmanMonteith() command.
</p>
*/
public class TranslatePenmanMonteith_Command extends AbstractCommand implements Command
{
...
*/
public TranslatePenmanMonteith_Command ()
{	super();
	setCommandName ( "TranslatePenmanMonteith" );
}
...
{	// The command will be modified if changed...
	return (new TranslatePenmanMonteith_JDialog ( parent, this )).ok();
}
...
throws InvalidCommandParameterException, CommandWarningException, CommandException
{
    ...
    try {
    	...
    	for (int i = 0; i < kbcListSize; i++) {
    		kpm = (StateCU_PenmanMonteith)kbcList.get(i);
    		kbc_name = kpm.getName();
    		if ( kbc_name.equalsIgnoreCase(OldCropType) ) {
    			// Matched so change the name...
    			kpm.setName ( NewCropType );
    			Message.printStatus ( 2, routine,
    			"Changed Penman-Monteith crop type \"" + OldCropType + "\" to \"" + NewCropType + "\"" );
    			++matchCount;
    		}
    	}
		...
		if ( matchCount == 0 ) {
			if ( IfNotFound.equalsIgnoreCase(_Ignore) ) {
				...
				message = "No Penman-Monteith crop types were matched: warning and not translating.";
				Message.printWarning(warning_level,
					MessageUtil.formatMessageTag( command_tag, ++warning_count),
					routine, message );
				status.addToLog ( CommandPhaseType.RUN,
				...
				message = "No Penman-Monteith crop types were matched: failing and not translating.";
				Message.printWarning(warning_level,
					MessageUtil.formatMessageTag( command_tag, ++warning_count),
					routine, message );
				status.addToLog ( CommandPhaseType.RUN,
        ...
        message = "Unexpected error tranlating Penman-Monteith crop types (" + e + ").";
        Message.printWarning ( warning_level, 
        MessageUtil.formatMessageTag(command_tag, ++warning_count),
        routine, message );
