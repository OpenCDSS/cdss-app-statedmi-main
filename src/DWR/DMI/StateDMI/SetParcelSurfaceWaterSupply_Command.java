// SetParcelSurfaceWaterSupply_Command - This class initializes, checks, and runs the SetParcelSurfaceWaterSupply() command.
...
/**
This class initializes, checks, and runs the SetParcelSurfaceWaterSupply() command.
*/
public class SetParcelSurfaceWaterSupply_Command extends AbstractCommand implements Command
{
...
Constructor.
*/
public SetParcelSurfaceWaterSupply_Command ()
{	super();
	setCommandName ( "SetParcelSurfaceWaterSupply" );
}
...
{
	return (new SetParcelSurfaceWaterSupply_JDialog ( parent, this )).ok();
}
...
{	String message, routine = getCommandName() + "_Command.runCommand";
	...
	try {
		...
		if ( parcel == null ) {
			...
			// Try to find the matching surface supply for the parcel.
			StateCU_SupplyFromSW supply = null;
			StateCU_SupplyFromSW supplyFromSW = null;
			for ( StateCU_Supply supply0 : parcel.getSupplyList() ) {
				if ( supply0 instanceof StateCU_SupplyFromSW ) {
					supplyFromSW = (StateCU_SupplyFromSW)supply0;
					if ( supplyFromSW.getID().equals(WDID) ) {
						supply = supplyFromSW;
						break;
					}
				}
			...
			if ( supply == null ) {
				// Create a new supply if requested.
				...
				// Parcel was found
				if ( IfFound.equalsIgnoreCase(_Warn) ) {
				...
				else if ( IfFound.equalsIgnoreCase(_Fail) ) {
					message = "Parcel for year " + year + ", parcel ID " + ParcelID +
				...
				else if ( IfFound.equalsIgnoreCase(_Remove) ) {
					// Remove the supply
					Message.printStatus(2, routine, "Removing existing surface water supply for year=" + year +
						" parcelID=" + ParcelID + " WDID=" + WDID);
        ...
        message = "Unexpected error setting parcel surface water supply (" + e + ").";
        Message.printWarning ( warning_level, 
