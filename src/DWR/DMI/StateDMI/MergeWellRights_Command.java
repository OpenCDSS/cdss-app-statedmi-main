...
*/
private List extractExplicitRights ( List<StateMod_WellRight> smrightsOrig )
{	List<StateMod_WellRight> extractedRights = new Vector();
...
*/
private List mergeRightsFromMultipleParcelYears ( List smrights_orig, String OutputFile_full, List OutputComments_List )
throws Exception
{	String routine = getClass().getName() + ".mergeRightsFromMultipleParcelYears";
	...
	List explicitRights = extractExplicitRights ( smrights_orig );
	...
	List smwells = StateMod_Util.getWaterRightLocationList ( smrights_orig, -1 );
	...
	List rights_loc_year1;	// All rights at a location for year 1
	List rights_loc_year2;	// All rights at a location for year 2
	List rights_year1;	// Rights from first parcel year (or previous merge), loc and right ID
	List rights_year2;	// Rights from second parcel year.
	StateMod_WellRight right1, right2;	// Rights to compare.
	String year1_string = "";	// For messages.
	String year2_string = "";	// For messages
	List smrights_prev = new Vector();
	
	List outputFileList = null;
	if ( OutputFile_full != null ) {
		outputFileList = new Vector();
	}
	for ( int iyear = 0; iyear < (parcel_years.length - 1); iyear++ ) {
		...
		if ( OutputFile_full != null ) {
			...
			PropList writeProps = new PropList("");
	       	...
	       	List year1Rights = StateMod_Util.getWaterRightsForLocation ( smrights_orig, null, parcel_years[iyear] );
		...
		for ( int iwell = 0; iwell < smwells_size; iwell++ ) {
			...
			List rightids_year1 = StateMod_Util.getWaterRightIdentifiersForLocation(
			...
			for ( int iright1_ids = 0; iright1_ids < rightids_year1_size; iright1_ids++ ) {
				...
				rights_year1 = StateMod_Util.getWaterRightsForLocationAndRightIdentifier(
					rights_loc_year1, null, (String)rightids_year1.get(iright1_ids), -1 );
				rights_year2 = StateMod_Util.getWaterRightsForLocationAndRightIdentifier(
					rights_loc_year2, null, (String)rightids_year1.get(iright1_ids), -1 );
				Message.printStatus ( 2, routine, "Merging rights for loc \"" +
					id + "\" right id=\"" + (String)rightids_year1.get(iright1_ids) +
					"\" from " + year1_string +
...
*/
private List<StateMod_Right> sumDecrees ( List<StateMod_Right> smrights )
{	String routine = getClass().getName() + ".sumDecrees";
	List smAllRightsSummed = new Vector();
	...
	for ( int iwell = 0; iwell < smwellIDsSize; iwell++ ) {
		...
		for ( int irightID = 0; irightID < smwellRightIDsSize; irightID++ ) {
			...
			List<StateMod_Right> smwellRights =
			...
			List<StateMod_Right> smwellRightsSummed = new Vector();
			for ( int iright = 0; iright < smwellRights.size(); iright++ ) {
				// Get the next right
				wer = (StateMod_WellRight)smwellRights.get(iright);
