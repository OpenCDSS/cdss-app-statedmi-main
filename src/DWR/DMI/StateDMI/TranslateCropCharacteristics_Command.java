// TranslateCropCharacteristics_Command - This class initializes, checks, and runs the TranslateCropCharacteristics() command.
...
/**
<p>
This class initializes, checks, and runs the TranslateCropCharacteristics() command.
</p>
*/
public class TranslateCropCharacteristics_Command extends AbstractCommand implements Command
{
...
*/
public TranslateCropCharacteristics_Command ()
{	super();
	setCommandName ( "TranslateCropCharacteristics" );
}
...
{	// The command will be modified if changed...
	return (new TranslateCropCharacteristics_JDialog ( parent, this )).ok();
}
...
throws InvalidCommandParameterException, CommandWarningException, CommandException
{
    ...
    try {
    	...
    	for (int i = 0; i < cchListSize; i++) {
    		...
    		cch_name = cch.getName();
    		if ( cch_name.equalsIgnoreCase(OldCropType) ) {
    			// Matched so change the name...
    			cch.setName ( NewCropType );
    			Message.printStatus ( 2, routine, "Changed crop characteristics crop type \"" +
    			OldCropType + "\" to \"" + NewCropType + "\"" );
    			++matchCount;
    		}
    	}
		...
		if ( matchCount == 0 ) {
			if ( IfNotFound.equalsIgnoreCase(_Ignore) ) {
				...
				message = "No crop characteristics were matched: warning and not translating.";
				Message.printWarning(warning_level,
					MessageUtil.formatMessageTag( command_tag, ++warning_count),
					routine, message );
				status.addToLog ( CommandPhaseType.RUN,
				...
				message = "No crop characteristics were matched: failing and not translating.";
				Message.printWarning(warning_level,
					MessageUtil.formatMessageTag( command_tag, ++warning_count),
					routine, message );
				status.addToLog ( CommandPhaseType.RUN,
        ...
        message = "Unexpected error tranlating crop characteristics crop types (" + e + ").";
        Message.printWarning ( warning_level, 
        MessageUtil.formatMessageTag(command_tag, ++warning_count),
        routine, message );
