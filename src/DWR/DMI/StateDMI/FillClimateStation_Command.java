// FillClimateStation_Command - This class initializes, checks, and runs the FillClimateStation() command.
...
/**
This class initializes, checks, and runs the FillClimateStation() command.
*/
public class FillClimateStation_Command extends AbstractCommand implements Command
{
...
Constructor.
*/
public FillClimateStation_Command ()
{	super();
	setCommandName ( "FillClimateStation" );
}
...
{	String routine = "FillClimateStation_Command.checkCommandParameters";
...
{	String routine = "FillClimateStation_Command.editCommand";
	...
	return (new FillAndSetClimateStation_JDialog ( parent, this, Region1_List, Region2_StringList )).ok();
}
...
{	String message, routine = getCommandName() + "_Command.runCommand";
	...
	try {
		...
		for (int i = 0; i < stationListSize; i++) {
			...
			if ( fill_latitude && StateCU_Util.isMissing(cli.getLatitude()) ) {
				Message.printStatus ( 2, routine, "Filling " + id + " latitude -> " + Latitude );
				cli.setLatitude ( latitude_double );
			}
			if ( fill_elevation && StateCU_Util.isMissing(cli.getElevation()) ) {
				Message.printStatus ( 2, routine, "Filling " + id + " elevation -> " + Elevation );
				cli.setElevation ( elevation_double );
			}
			if ( fill_region1 && StateCU_Util.isMissing(cli.getRegion1()) ) {
				Message.printStatus ( 2, routine, "Filling " + id + " Region1 -> " + Region1 );
				cli.setRegion1 ( Region1 );
			}
			if ( fill_region2 && StateCU_Util.isMissing(cli.getRegion2()) ) {
				Message.printStatus ( 2, routine, "Filling " + id + " Region2 -> " + Region2 );
				cli.setRegion2 ( Region2 );
			}
			if ( fill_name && StateCU_Util.isMissing(cli.getName()) ) {
				Message.printStatus ( 2, routine, "Filling " + id + " Name -> " + Name );
				cli.setName ( Name );
			}
			if ( fill_zh && StateCU_Util.isMissing(cli.getZh()) ) {
				Message.printStatus ( 2, routine, "Filling " + id +
					" height of humidity/temperature measurements -> " + HeightHumidityMeas );
				cli.setZh ( zh_double );
			}
			if ( fill_zm && StateCU_Util.isMissing(cli.getZm()) ) {
				Message.printStatus ( 2, routine, "Filling " + id +
