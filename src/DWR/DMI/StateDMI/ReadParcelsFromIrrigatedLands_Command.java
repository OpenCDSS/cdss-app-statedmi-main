...
throws InvalidCommandParameterException, CommandWarningException, CommandException {
	...
	// Loop through surface water supplies and determine the column numbers for surface supplies.
   	...
   	List<Integer> swIdColNumList = new ArrayList<>();
	...
	// Loop through groundwater supplies and determine the column numbers for groundwater supplies.
   	...
   	List<Integer> gwIdColNumList = new ArrayList<>();
   	List<Integer> gwIdTypeColNumList = new ArrayList<>();
	...
	// TODO smalers 2023-07-27 need to check only selected commands if only selected are run.
	    ...
	    try {
	    	...
	    	for ( int irec = 0; irec < table.getNumberOfRecords(); irec++ ) {
	    		...
	    		if ( doAddParcel ) {
	    			...
	    			// Also add the parcel to each CU Location (supply).
	    			for ( StateCU_Supply supply : parcel.getSupplyList() ) {
	    				StateCU_Location culoc = StateCU_Location.lookupForId(culocList, supply.getID() );
	    				if ( culoc != null ) {
	    					// TODO smalers 2023-07-27 need to handle collections:
	    					// - for now only handle explicit matches
