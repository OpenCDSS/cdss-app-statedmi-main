...
CommandWarningException, CommandException {
	...
	try {
		...
		// find the irrigation practice time series object for the location and set the data.
		...
		// Booleans that indicate if specific data values should be set because column was specified.
		...
		String cds_id, id;
		// Used with SetFlagDescription:
		// - only set the description once
		boolean flagDescriptionHasBeenSet = false;
		...
		for ( int irec = 0; irec < tsize; irec++ ) {
			...
			if ( ProcessWhen_int == Now_int ) {
				...
				cds = CUCropPatternTS_List.get(cdsPos);
			}
					...
					if ( doSet ) {
						...
						if ( ProcessWhen_int == Now_int ) {
							...
							setCropPatternTS (
								...
								SetFlag );
							if ( !flagDescriptionHasBeenSet ) {
								// Set the flag description only one time.
								cds.setDataFlagDescription ( SetFlag, SetFlagDescription );
								flagDescriptionHasBeenSet = true;
							}
						}
						else if ( ProcessWhen_int == WithParcels_int ) {
							// This is used with legacy approach.
							// Instead of setting the data, save supplemental records for use with the readCropPatternTSFromHydroBase command.
...
@param setFlag flag to set on time series value to indicate being set
*/
private void setCropPatternTS ( StateCU_Location culoc, StateCU_CropPatternTS cds, int setstart, int setend, String crop_type,
	double area, HashMap<String,Boolean> idYearHasBeenProcessedMap, String setFlag )
throws Exception {
	// Now reset to new data.  The number of crops does not need to match the original.
	// First make sure the crop time series is available.
...
/**
Set the crop pattern time series to zero.
This is typically done the first time that a location is encountered.
